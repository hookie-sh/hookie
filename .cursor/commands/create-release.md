...
7. **Tag and push (only after confirmation)**
   ...
   - After a successful push, remind the user that the "Release CLI" workflow will run and that `CLERK_PUBLISHABLE_KEY` must be set in repo secrets. For npm install support, also set `NPM_AUTH_TOKEN`. The release notes (changelog) are generated by GoReleaser from conventional commits and appear on the GitHub release page.

## Notes

- Releases are only allowed from **main**. The tag is created at the current `HEAD` on main.
- The release workflow publishes six npm packages: the main `@hookie-sh/hookie` plus five platform-specific packages (`@hookie-sh/hookie-darwin-arm64`, etc.). For the first release after changing the release flow, validate on a pre-release tag (e.g. `v1.0.0-optional.1`) before cutting a stable release.
